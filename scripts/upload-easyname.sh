...
if [[ ! "$TAG" =~ ^arome-[0-9]{8}T[0-9]{4}Z$ ]]; then
	echo "Ungültiger AROME-Tag: ${TAG}" >&2
	exit 1
fi
...
echo "Easyname-Upload vollständig: ${REMOTE_TAG_DIR}"
echo "Aktiver Lauf: ${REMOTE_BASE}/latest.json"
...
LFTP
	...
	echo "Remote-Verzeichnis konnte für die Bereinigung nicht gelesen werden; Upload bleibt gültig." >&2
	exit 0
}
...
for old_tag in "${REMOTE_TAGS[@]:$RETAINED_RUNS}"; do
	echo "Entferne alten Easyname-Lauf: ${old_tag}"
...
LFTP
	then
		echo "Alter Easyname-Lauf konnte nicht entfernt werden: ${old_tag}" >&2
	fi
done
